...
<tr>
<td> <button onclick="window.location.href='addaccount.jsp?accountID=<%=account.getAccountID()%>&customerID=<%=account.getCustomerID()%>&branch=<%=account.getBranchName()%>&bal=<%=account.getBankBalance()%>';"><%out.print(account.getAccountID()); %></button></td>
